...
import { CreateContextOptions, useContextMenu } from '../../context-menu';
...
export default defineComponent({
	...
	setup(props, { attrs, slots, emit, expose }) {
		...
		const getNotFound = computed(
			(): boolean =>
				!getTreeSearchData.value || getTreeSearchData.value.length === 0
		);
		...
		onMounted(() => {
			const level = parseInt(props.defaultExpandLevel as string, 10);
			if (level > 0) {
		...
		const instance: TreeActionType = {
			...
			getSearchValue: () => searchState.searchText,
		};
		...
		function renderAction(node: TreeItem) {
			...
			return actionList.map((item, index) => {
				...
				return (
					<span key={index} class={ns.e('action')}>
						{item.render(node)}
					</span>
				);
			});
		...
		const treeData = computed(() => {
			...
			eachTree(data, (item) => {
				...
				const titleDom = isHighlight ? (
					<span
						class={unref(getBindValues)?.blockNode ? `${ns.e('content')}` : ''}
					>
				...
				const iconDom = icon ? (
					...
					<span style={'margin-right: 7px'}>{getSlot(slots, 'icon')}</span>
				) : null;
					...
					>
						{slots?.title ? (
							...
							<>
								{iconDom}
								{titleDom}
								<span class={ns.e('actions')}>{renderAction(item)}</span>
							</>
						)}
					</span>
				);
