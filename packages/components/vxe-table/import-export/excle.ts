...
function setExcelCellStyle(
	...
	align?:
		| VxeTablePropTypes.Align
		| VxeTablePropTypes.HeaderAlign
		| VxeTablePropTypes.FooterAlign
) {
...
function exportXLSX(
			...
			colgroups.forEach((cols, rIndex) => {
				...
				cols.forEach((column) => {
					...
					const headExportMethod =
						(column as any).headerExportMethod ||
						(columnOpts as any).headerExportMethod;
	...
	const exportMethod = () => {
		const workbook: ExcelJS.Workbook = new (
			globalExcelJS || (window as any).ExcelJS
		).Workbook();
...
function importError(
	params: VxeGlobalInterceptorHandles.InterceptorImportParams
) {
	const { modal, t } = globalVxetable;
	...
	fileReader.onload = (evnt) => {
		...
		if (readerTarget) {
			workbook.xlsx.load(readerTarget.result as ArrayBuffer).then((wb) => {
				...
				if (firstSheet) {
					const sheetValues = Array.from(
						firstSheet.getSheetValues()
					) as string[][];
					const fieldIndex = XEUtils.findIndexOf(
						sheetValues,
					...
					if (status) {
						const records = sheetValues.slice(fieldIndex + 1).map((list) => {
							...
							tableFields.forEach((field) => {
								record[field] = XEUtils.isUndefined(item[field])
									? null
									: item[field];
							});
							return record;
...
export const VXETablePluginExportXLSX = {
	install(
		...
		if (!/^(4)\./.test(vxetable.version) && !/v4/i.test((vxetable as any).v)) {
			...
			console.error(
				'[vxe-table-plugin-export-pdf 4.x] Version vxe-table 4.x is required'
			);
		}
